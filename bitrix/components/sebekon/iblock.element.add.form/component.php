...
if(!CModule::IncludeModule("sebekon.psbpayment")) {
	return;
}
...
$key = array_search('NAME', $arParams['PROPERTY_CODES']);
if($key !== false) {
	unset($arParams['PROPERTY_CODES'][$key]);
}

$key = array_search('NAME', $arParams['PROPERTY_CODES_REQUIRED']);
if($key !== false) {
...
if ($bAllowAccess)
{
	...
	while ($arProperty = $rsIBLockPropertyList->GetNext())
	{
		...
		if(!in_array($arProperty['CODE'], $arExceptionProps)) {
			
			// add property to edit-list
			if (in_array($arProperty["ID"], $arParams["PROPERTY_CODES"]) || $arProperty["CODE"] == 'ORDER_SUM' || $arProperty["CODE"] == 'CARD_NUMBER') {
				$arResult["PROPERTY_LIST"][] = $arProperty["ID"];
				
				if(($arProperty["CODE"] == 'ORDER_SUM' || $arProperty["CODE"] == 'CARD_NUMBER') && !in_array($arProperty["ID"], $arParams['PROPERTY_CODES_REQUIRED'])) {
					$arParams['PROPERTY_CODES'][] = $arProperty["ID"];
			...
			$key = array_search($arProperty["ID"], $arParams['PROPERTY_CODES']);
			if($key !== false) {
				unset($arParams['PROPERTY_CODES'][$key]);
			}
			
			$key = array_search($arProperty["ID"], $arParams['PROPERTY_CODES_REQUIRED']);
			if($key !== false) {
				unset($arParams['PROPERTY_CODES_REQUIRED'][$key]);
			}
		...
		if($arProperty["CODE"] == 'ORDER_SUM' || $arProperty["CODE"] == 'ORDER_STATUS') {
			$arResult['SEBEKON_DATA'][$arProperty["CODE"] . '_PROP_ID'] = $arProperty["ID"];
		}
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		$arUpdateValues = array('NAME' => date('d.m.Y H:i:s'));
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			else
			{
				...
				if($arStatus = CIBlockProperty::GetPropertyEnum('ORDER_STATUS', array(), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'EXTERNAL_ID' => 1))->Fetch()) {
					$arUpdateValues["PROPERTY_VALUES"][$arResult['SEBEKON_DATA']['ORDER_STATUS_PROP_ID']] = $arStatus['ID'];
				}
				...
				if (!$arParams["ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
					...
					$arEventData = array(
						'ORDER_ID' => $arParams["ID"],
						'MESSAGE' => GetMessage('EVENT_MESSAGE', array('#ORDER_SUM#' => number_format($arUpdateValues['PROPERTY_VALUES'][$arResult['SEBEKON_DATA']['ORDER_SUM_PROP_ID']], 2, ',', ' ')))
					);
					CEvent::SendImmediate(CSebekonPsbpayment::EVENT_TYPE, SITE_ID, $arEventData);					
					
					
					$ps_key = trim($arParams['PS_KEY']); // !!
					$ps_url = trim($arParams['PS_URL']); // http://193.200.10.117:8080/cgi-bin/cgi_link
					//$ps_url = 'http://193.200.10.117:8080/cgi-bin/cgi_link';
					$ps_amount = trim($arUpdateValues['PROPERTY_VALUES'][$arResult['SEBEKON_DATA']['ORDER_SUM_PROP_ID']]);
					$ps_order = trim($arParams["ID"]);
					$ps_desc = trim($arParams['PS_DESC']);
					$ps_currency = trim($arParams['PS_CURRENCY']); // RUB
					$ps_terminal = trim($arParams['PS_TERMINAL']);
					$ps_merchant = trim($arParams['PS_MERCHANT']);
					$ps_merchant_name = trim($arParams['PS_MERCHANT_NAME']);
					$ps_email = $arParams['PS_EMAIL'];
					$ps_back = trim($arParams['PS_BACK']); // http://localhost:6448/personal/order/payment/psbank.php
					...
					if (stripos($ps_back,'?')===FALSE) {
						$ps_back .= "?";
					} elseif(substr($ps_back, -1) != '&') {
						$ps_back .= '&';
					}
					
					$ps_back .= "RETURN_FROM_BANK=Y&ORDER_ID=".intval($ps_order);
					...
					foreach ($arParams['PROPERTY_CODES'] as $v) {
						if (is_numeric($v)) {
							$_name = $arResult["PROPERTY_LIST_FULL"][$v]["CODE"];
							if ($arResult["PROPERTY_LIST_FULL"][$v]["PROPERTY_TYPE"] == "E") {
								$prop = CIblockElement::GetById($arUpdatePropertyValues[$v][0])->Fetch();
								$str_replaces['#'.$_name.'#'] = $prop["NAME"];
							}
							elseif ($arResult["PROPERTY_LIST_FULL"][$v]["PROPERTY_TYPE"] == "G") {
								$prop = CIblockSection::GetById($arUpdatePropertyValues[$v][0])->Fetch();
								$str_replaces['#'.$_name.'#'] = $prop["NAME"];
							}
							else
								$str_replaces['#'.$_name.'#'] = $arUpdatePropertyValues[$v];
						} else {
							$str_replaces['#'.$v.'#'] = $arUpdateValues[$v];
						}
					}
					$ps_desc =str_replace(array_keys($str_replaces), $str_replaces, $ps_desc);
					
					// add the start of the vpcURL querystring parameters
					$data = array(
						"AMOUNT" =>$ps_amount,
						"CURRENCY" =>$ps_currency,
						"ORDER" => date("Ymd",strtotime($arUpdateValues['NAME'])).$ps_order,
						"DESC" => GetMessage("SEBEKON_PSBPAYMENT_ZAKAZ").$ps_order.' '.$ps_desc,
					...
					if(ToUpper(SITE_CHARSET)!="UTF-8"){
						$data['DESC'] = iconv(SITE_CHARSET, "UTF-8", $data['DESC']);
					}
