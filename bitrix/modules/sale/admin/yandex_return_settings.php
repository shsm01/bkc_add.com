...
if (($request->getPost("Update") || $request->getPost("Apply")) && check_bitrix_sessid())
{
	...
	if ($sitesData)
	{
		...
		{
			if ($fields["SETTINGS_CLEAR"] || $fields["SETTINGS_CLEAR_ALL"])
			{
				$all = array_key_exists('SETTINGS_CLEAR_ALL', $fields);
				PaySystem\YandexCert::clear($ID, $personTypeId, $all);
				continue;
			}
			...
			<tr>
				...
				<td width="60%" class="adm-detail-content-cell-r">
					<?if (!PaySystem\YandexCert::isLoaded($ID, $personTypeId)):?>
						<input type="file" name="CERT_FILE_<?=$personTypeId;?>" size="40"><br>
						<?=Loc::getMessage('SALE_YANDEX_RETURN_TEXT_CLEAR_ALL')?>
						<input id='settings[<?=$personTypeId;?>][SETTINGS_CLEAR_ALL]' type="checkbox" name='settings[<?=$personTypeId;?>][SETTINGS_CLEAR_ALL]'>
					<?else:?>
