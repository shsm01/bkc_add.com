...
class CAllSaleOrderChange
{
	 ...
	 * @return bool
	 */
	public static function AddRecord($orderId, $type, $data = array(), $entity = null, $entityId = null)
	{
		global $USER;
	 ...
	 * @return bool
	 */
	public static function AddRecordsByFields($orderId, $arOldFields, $arNewFields, $arDeleteFields = array(), $entity = "", $entityId = null)
	{
		if ($orderId <= 0)
			return false;
		...
		foreach ($arNewFields as $key => $val)
		{
			...
			if (!array_key_exists($key, $arOldFields) || (array_key_exists($key, $arOldFields) && strlen($val) > 0 && $val != $arOldFields[$key]) && !in_array($key, $arDeleteFields))
			{
				...
				if ($arRecord)
					CSaleOrderChange::AddRecord($orderId, $arRecord["TYPE"], serialize($arRecord["DATA"]), $entity, $entityId);
			}
		}
