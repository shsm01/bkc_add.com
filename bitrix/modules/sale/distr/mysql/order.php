...
class CSaleOrder extends CAllSaleOrder
{
	function Add($arFields)
	{
		...
		if ($isOrderConverted == 'Y')
		{
			if (!empty($arFields1))
			{
				$arFields1 = \Bitrix\Sale\Compatible\OrderCompatibility::backRawField($arFields1);
			}
	...
	function GetList($arOrder = array("ID"=>"DESC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
	{
		...
		if ($isOrderConverted == "Y")
		{
			$result = \Bitrix\Sale\Compatible\OrderCompatibility::getList($arOrder, $arFilter, $arGroupBy, $arNavStartParams, $arSelectFields);
