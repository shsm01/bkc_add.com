				...
				<select multiple="multiple" size="5" name="DELIVERY_HANDLERS[]">
					<?
						$dbDelivery = \Bitrix\Sale\Delivery\Services\Table::getList(array(
							'order' => array("SORT" => "ASC", "PARENT_NAME"=> "ASC", "NAME"=>"ASC"),
							'filter' => array(
								"ACTIVE" => "Y",
								"!=CLASS_NAME" => array(
							...
							'select' => array('ID', 'NAME', 'PARENT', 'PARENT_ID', 'PARENT_NAME' => 'PARENT.NAME'),
						));

						while ($arDelivery = $dbDelivery->fetch())
						{
							echo '<option value="'.$arDelivery["ID"].'"'
								.(is_array($deliveryIds) && in_array($arDelivery["ID"], $deliveryIds) ? " selected" : "").'>'.
								(strlen($arDelivery["PARENT_NAME"]) > 0 ? htmlspecialcharsbx($arDelivery["PARENT_NAME"]).":" : "").
								htmlspecialcharsbx($arDelivery["NAME"]).' ['.$arDelivery["ID"].']'.
							'</option>';
						}
