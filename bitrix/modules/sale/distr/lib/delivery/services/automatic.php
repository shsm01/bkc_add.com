...
class Automatic extends Base
{
				...
				"MAIN" => array(
					...
					"ITEMS" => array (
						"SID" => array(
							...
							"OPTIONS" => $handlers,
							"ONCHANGE" => "top.BX.showWait(); this.form.submit();"
						)
					)
				)
			);
	...
	protected static function convertNewOrderToOld(\Bitrix\Sale\Shipment $shipment)
	{
		...
		if ($newOrder->getBasket() !== false)
		{
			/** @var BasketItem $baskeItem */
			foreach($newOrder->getBasket() as $baskeItem)
			{
				$oldOrder["ITEMS"][] = $baskeItem->getFieldValues();
				$itemWeight = $baskeItem->getField("WEIGHT");

				if(floatval($itemWeight) > 0)
					$oldOrder["WEIGHT"] += floatval($itemWeight);
			}
		}
		...
		if($arHandlersList === null)
		{
			...
			foreach(GetModuleEvents("sale", "onSaleDeliveryHandlersBuildList", true) as $arHandler)
			{
				...
				if(strlen($indexBy) > 0 && isset($initParams[$indexBy]))
					$arHandlersList[$initParams[$indexBy]] = $initParams;
				else
					$arHandlersList[] = $initParams;
			}
		}
	...
	public static function convertNewServiceToOld($service, $sid = false)
	{
		...
		$service["CONFIG"] = self::createConfig($handlers[$service["SID"]], $service["SETTINGS"], $service["SITE_ID"]);
		...
		if(isset($service["ID"]) && intval($service["ID"]) > 0)
		{
			foreach(Manager::getByParentId($service["ID"]) as $profile)
			{
				...
				$restrictions = Manager::getRestrictionsByDeliveryId($service["ID"]);
	...
	public function getProfilesDefaultParams(array $fields = array())
	{
		...
		{
			$profName =  isset($params["TITLE"]) ? $params["TITLE"] : "";

			if(!empty($fields["NAME"]))
				$profName = $fields["NAME"]." (".$profName.")";
