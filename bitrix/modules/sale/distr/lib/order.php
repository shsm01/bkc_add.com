	...
	extends OrderBase implements \IShipmentOrder, \IPaymentOrder
{
	...
	public function onShipmentCollectionModify($action, Shipment $shipment, $name = null, $oldValue = null, $value = null)
	{
		...
		if ($name == "ALLOW_DELIVERY")
		{
			...
			if ($r->isSuccess())
			{
				$event = new Main\Event('sale', EventActions::EVENT_ON_SHIPMENT_DELIVER, array($shipment));
				$event->send();
			}
			else
			{
	...
	public function save()
	{
		...
		$event = new Main\Event('sale', EventActions::EVENT_ON_ORDER_BEFORE_SAVED, array($this));
		$event->send();
		...
		else
		{
			...
			OrderHistory::addAction('ORDER', $id, 'ORDER_ADDED', $id);
		}
		...
		$event = new Main\Event('sale', EventActions::EVENT_ON_ORDER_SAVED, array($this, $this->isNew));
		$event->send();
	...
	public static function delete($id)
	{
		...
		$event = new Main\Event('sale', EventActions::EVENT_ON_BEFORE_ORDER_DELETE, array($order));
		$event->send();
		...
		$event = new Main\Event('sale', EventActions::EVENT_ON_ORDER_DELETED, array($order, ((bool) $r->isSuccess()) ));
		$event->send();
	...
	public function onBasketModify($action, BasketItem $basketItem, $name = null, $oldValue = null, $value = null)
	{
		...
		if ($name == "QUANTITY")
		{
			...
			$result = $this->setField(
				"PRICE",
				$this->getField("PRICE") + ($value - $oldValue) * $basketItem->getPrice()
			);
		...
		elseif ($name == "PRICE")
		{
			...
			$result = $this->setField(
				"PRICE",
				$this->getField("PRICE") + ($value - $oldValue) * $basketItem->getQuantity()
			);
	...
	protected function onAfterSyncPaid($oldPaid = null)
	{
		...
		{
			Internals\EventsPool::addEvent($this, EventActions::EVENT_ON_ORDER_PAID, array($this, $this->isPaid()));

			Internals\EventsPool::addEvent($this, EventActions::EVENT_ON_ORDER_PAID_SEND_MAIL, array($this, $this->isPaid()));
		}
	 ...
	 * @return Result
	 */
	public function onAfterDiscount(array $data)
	{
		if (!empty($data['BASKET_ITEMS']) && is_array($data['BASKET_ITEMS']))
		{
			...
			foreach ($data['BASKET_ITEMS'] as $basketCode => $basketItemData)
			{
				if ($basketItem = $basket->getItemByBasketCode($basketCode))
				{
					...
					if (isset($basketItemData['PRICE']) && floatval($basketItemData['PRICE']) >= 0
						&& $basketItem->getPrice() != floatval($basketItemData['PRICE']))
					{
						$basketItemData['PRICE'] = roundEx(floatval($basketItemData['PRICE']), SALE_VALUE_PRECISION);
...
//				return $result;
			...
			{
				/** @var Result $r */
				$r = $this->onAfterDiscount($discountData);
				if (!$r->isSuccess())
				{
					$result->addErrors($r->getErrors());
					return $result;
			...
			if (!$this->isExternal())
			{
				...
				if (isset($taxResult['TAX_PRICE']) && floatval($taxResult['TAX_PRICE']) >= 0)
				{
					if (!$this->isUsedVat())
					{
						$oldTaxPrice = (float)$this->getField('TAX_VALUE');

						if (floatval($this->getField('TAX_PRICE')) == 0 && floatval($this->getField('TAX_VALUE')) > 0)
						{
							$this->setField('TAX_PRICE', $oldTaxPrice);
						}

						if ($taxResult['TAX_PRICE'] != $oldTaxPrice)
						{
							...
							$this->setFieldNoDemand(
								"PRICE",
								$this->getField("PRICE") + ($taxResult['TAX_PRICE'] - $oldTaxPrice)
							);
						}
					}
				...
				{
					$this->setField('TAX_VALUE', $this->isUsedVat()? $this->getVatSum() : $this->getField('TAX_PRICE'));
				}
			}
