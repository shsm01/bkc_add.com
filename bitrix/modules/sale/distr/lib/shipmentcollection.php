	...
	extends Internals\EntityCollection
{
	...
	public function deleteItem($index)
	{
		...
		if ($oldItem->getId() > 0 && ($systemShipment = $this->getSystemShipment()) && $systemShipment->getId() == 0)
		{
...
//	}
	...
	public function save()
	{
		...
		foreach ($itemsFromDb as $k => $v)
		{
			...
			if ($order->getId() > 0)
			{
				OrderHistory::addAction('SHIPMENT', $order->getId(), 'SHIPMENT_REMOVE', $k);
			}
...
//	}
	...
	public function tryReserve()
	{
		...
		foreach ($this->collection as $shipment)
		{
			if ($shipment->isReserved() || $shipment->isSystem())
				continue;
