...
/*			if ($orderMode == Order::SALE_ORDER_CALC_TYPE_REFRESH)
	...
	protected function calculateMixed()
	{
		...
		foreach ($this->orderData['BASKET_ITEMS'] as $code => $basketItem)
		{
			if (!$this->isNewBasketItem($basketItem))
				continue;
			$this->orderData['BASKET_ITEMS'][$code]['BASE_PRICE'] = (
				isset($this->basketBasePrice[$code])
				? $this->basketBasePrice[$code]
				: $this->orderData['BASKET_ITEMS'][$code]['PRICE'] + $this->orderData['BASKET_ITEMS'][$code]['DISCOUNT_PRICE']
			);
		}
...
/*			if (!$this->newOrder)
	...
	protected function getApplyPrices()
	{
		$delivery = array(
			...
			'DISCOUNT' => (!isset($this->orderData['CUSTOM_PRICE_DELIVERY']) || $this->orderData['CUSTOM_PRICE_DELIVERY'] != 'Y' ? $this->orderData['PRICE_DELIVERY_DIFF'] : 0)
		);
		$basket = array();
		if (!empty($this->orderData['BASKET_ITEMS']))
		{
			foreach ($this->orderData['BASKET_ITEMS'] as $basketCode => $basketItem)
			{
				$basket[$basketCode] = array(
					...
					'DISCOUNT' => (!$this->isCustomPrice($basketItem) ? $basketItem['BASE_PRICE'] - $basketItem['PRICE'] : 0)
				);
			}
