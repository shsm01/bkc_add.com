	...
	protected function parseBasketItems(Sale\Basket $basket, array $requestBasketItems)
	{
		...
		foreach ($requestBasketItems as $basketIndex => $basketItemData)
		{
			...
			if (isset($basketItemData['ID']) && intval($basketItemData['ID']) > 0)
			{
				...
				$basketItem = $basket->getItemById($basketItemData['ID']);
			}
			...
			if (!($basketFields = $this->parseRawFields($basketFields)))
			{
				throw new Main\ArgumentNullException('index');
			}
			...
			else
			{
				...
				if ($isBasketItemCanBeAdded)
				{
					...
					if ($foundedBasketItem = $basket->getExistsItem($basketItem->getField('MODULE'), $basketItem->getProductId(), $propList))
					{
...
//			'USER_ID' => 'USER_ID',
	...
	{
		$fields = array_keys(BasketTable::getEntity()->getScalarFields());
...
//			}
	...
	protected function parseRawFields(array $fields, array $availableFields = array())
	{
		...
		$index = isset($fields['BASKET_CODE']) && strval($fields['BASKET_CODE'] != '') ? ($fields['BASKET_CODE']) : false;

		if ($index === false)
		{
			throw new Main\ArgumentNullException('BASKET_CODE');
		}
		...
		foreach ($fields as $name => $value)
		{
			...
			if ($firstLetter == "~" || $firstLetter == "=")
			{
				...
				$this->rawFields[$index][$firstLetter.$fieldName] = $value;

				unset($fields[$name]);
			}
		}

		unset($fields['BASKET_CODE']);
		...
		foreach ($this->rawFields as $basketCode => $rawFields)
		{
			...
			if ($basketItem = $basket->getItemById($basketCode))
			{
				...
				if (!empty($queryValue))
				{
					...
					{
						$result->addError(new Sale\ResultError(Main\Localization\Loc::getMessage('SALE_BASKET_COMPATIBLE_RAW_FIELD_UPDATE_ERROR', array(
							'#BASKET_ID#' => $basketItem->getId(),
							'#PRODUCT_NAME#' => $basketItem->getField('NAME'),
						)), 'SALE_BASKET_COMPATIBLE_RAW_FIELD_UPDATE_ERROR'));
						return $result;
					}
				}
			}
