...
//			$basket->setFUserId($fuserId);
	...
	public function onItemModify(BasketItem $item, $name = null, $oldValue = null, $value = null)
	{
		...
		if ($name == "QUANTITY")
		{
			...
			else
			{
				...
				{
					$result->addError( new ResultError(Loc::getMessage('BASKET_ITEM_WRONG_AVAILABLE_QUANTITY'), 'BASKET_ITEM_WRONG_AVAILABLE_QUANTITY') );
					return $result;
				}
			}
	...
	protected function loadFromDb(array $filter)
	{
		...
		$select = array("ID", "LID", "MODULE", "PRODUCT_ID", "QUANTITY", "WEIGHT",
			"DELAY", "CAN_BUY", "PRICE", "CUSTOM_PRICE", 'PRODUCT_PRICE_ID', "CURRENCY", 'BARCODE_MULTI', "RESERVED", "RESERVE_QUANTITY",
...
//				$this->setFUserId($basketItem->getFUserId());
	...
	public function getItemById($id)
	{
		...
		if ($index === false && $parentId === false)
		{
			return false;
		}

		if ($index !== false && isset($this->collection[$index]))
		{
		...
		if ($parentId)
		{
			...
			if ($parentIndex !== false && isset($this->collection[$parentIndex]))
			{
				/** @var BasketItem $item */
	...
	public function save()
	{
		...
		if (!$order)
		{
			...
			$event = new Main\Event('sale', EventActions::EVENT_ON_BASKET_BEFORE_SAVED, array($this));
			$event->send();
		...
		{
			/** @var Main\Entity\Event $event */
			$event = new Main\Event('sale', EventActions::EVENT_ON_BASKET_SAVED, array($this));
			$event->send();
