		...
		foreach ($poolQuantities as $basketCode => $quantity)
		{
			...
			if ($r->isSuccess())
			{
				...
				if ((!empty($reserveBasketItemResult) && is_array($reserveBasketItemResult))
					&& (array_key_exists('QUANTITY', $reserveBasketItemResult))
				)
				{
			...
			{
//				throw new SystemException("reserveBasketItem");
			...
			if (Configuration::isEnabledReservation() && $quantity != $reservedQuantity)
			{
...
//		{
//			$this->setFieldNoDemand('RESERVED', $orderReserved ? 'Y' : 'N');
//		}
...
//		$reservedBasketList = static::getReserveBasketListFromShipmentItemCollection($shipmentItemCollection);
					...
					foreach ($providerBasketItemList as $providerBasketItem)
					{
						...
						if (Configuration::useStoreControl())
						{
							...
							if (!empty($basketStoreData))
							{
								...
								if ($quantity > $allBarcodeQuantity)
								{
									$resultProductData->addError(new ResultError(Loc::getMessage('SALE_PROVIDER_SHIPMENT_SHIPPED_LESS_QUANTITY', array(
										'#PRODUCT_NAME#' => $providerBasketItem['BASKET_ITEM']->getField('NAME')
									)), 'SALE_PROVIDER_SHIPMENT_SHIPPED_LESS_QUANTITY'));
								}
								elseif ($quantity < $allBarcodeQuantity)
								{
									$resultProductData->addError(new ResultError(Loc::getMessage('SALE_PROVIDER_SHIPMENT_SHIPPED_MORE_QUANTITY', array(
										'#PRODUCT_NAME#' => $providerBasketItem['BASKET_ITEM']->getField('NAME')
									)), 'SALE_PROVIDER_SHIPMENT_SHIPPED_MORE_QUANTITY'));
								}
							}
						...
						if ($resultProductData->isSuccess())
						{
							...
							if ($needShip === true)
							{
								if (method_exists($provider, 'tryShipmentProduct'))
								{
									...
									$resultProductData = $provider::tryShipmentProduct($providerBasketItem['BASKET_ITEM'], $providerBasketItem['RESERVED'], $basketStoreData, $quantity);
								}
//								else
//								{
//									/** @var Result $resultProductData */
...
//								}
							...
							else
							{
								...
								{
									/** @var Result $resultProductData */
									$resultProductData = $provider::tryUnshipmentProduct($providerBasketItem['PRODUCT_ID']);
								}
//								else
//								{
//									/** @var Result $resultProductData */
...
//				$tryShipmentBundle = false;
				foreach($bundleChildList as $bundleChildBasketCode)
				{
					...
					{
//						$tryShipmentBundle = new Result();
//						$tryShipmentBundle->addError(new ResultError('Bundle child item not found'));
//						break;
...
//				$resultList[$bundleParentBasketCode] = $tryShipmentBundle;
			}
...
//			else
//			{
//				$result->addError(new ResultError(Loc::getMessage('SALE_PROVIDER_STORE_DATA_BARCODE_NOT_FOUND', array(
//					'#PRODUCT_NAME#' => $basketItem->getField('NAME')
//				)), 'SALE_PROVIDER_STORE_DATA_BARCODE_NOT_FOUND'));
//			}
...
//				$result->addError(new ResultError($e->getMessage(), $e->getCode()) );
...
//			$availableQuantity -= ($poolQuantity === null ? ($quantity - $availableQuantity) * -1 : $poolQuantity);
		}
		else
		{
...
//					{
//						$shipment->setMark();
//						$result = $shipmentItem->getReservedQuantity() + ($shipmentItem->getQuantity() - $shipmentItem->getReservedQuantity()) -  $resultReserveData['QUANTITY_NOT_RESERVED'];
//					}
//
...
//	 * @throws NotSupportedException
//	 */
//	public static function unreserveBasketItem(BasketItem $basketItem, $quantity)
//	{
//		$result = array();
//
//		$provider = $basketItem->getProvider();
...
//			throw new NotSupportedException('provider not supported');
//		}
//		elseif (array_key_exists("IBXSaleProductProvider", class_implements($provider)))
//		{
//
//			$data = array(
//				"PRODUCT_ID" => $basketItem->getProductId(),
...
//				if ($resultReserveData['RESULT'])
//				{
//					$result = $resultReserveData['QUANTITY_RESERVED'];
//
////					if (isset($resultReserveData['QUANTITY_NOT_RESERVED']) && floatval($resultReserveData['QUANTITY_NOT_RESERVED']) > 0)
////					{
////						$shipment->setMark();
...
//				else
//				{
//					new SystemException('unreserve error');
//				}
//
//			}
//
...
//	}
	 ...
	 * @throws NotSupportedException
	 */
	private static function unreserveShipment(Shipment $shipment)
	{
		$result = array();
		...
		foreach ($shipmentItemCollection as $shipmentIndex => $shipmentItem)
		{
			$resultShipmentItem = static::unreserveShipmentItem($shipmentItem, $shipmentItem->getReservedQuantity());

			$resultShipmentItem = -$shipmentItem->getReservedQuantity();

			$result[$shipmentItem->getBasketCode()] = $resultShipmentItem;
		}
				...
				if (!$shipment->isSystem())
				{
					...
					if (floatval($shipmentReservedQuantity) != floatval($shipmentItem->getQuantity()))
					{
						...
						if ($systemShipmentItem = $systemShipmentItemCollection->getItemByBasketCode($shipmentItem->getBasketCode()))
						{
							...
							{
//								static::$poolProductQuantity[$basketCode] += $quantityNeedReserve;
//								$systemShipmentItem->setField('RESERVED_QUANTITY', $systemShipmentItem->getReservedQuantity() - $quantityNeedReserve);
...
//					$result = $provider::getProductData($siteId, $userId, $currency, $providerBasketItemList, $select);
		...
		{
			$result->setData(array('AVAILABLE_QUANTITY' => $basketItem->getQuantity()));
			return $result;
		}
		...
		{
			$result->setData(array('AVAILABLE_QUANTITY' => $resultProductData['QUANTITY']));
		}
