<?
$MESS["T_IBLOCK_DESC_NEWS_DATE"] = "Выводить дату элемента";
$MESS["T_IBLOCK_DESC_NEWS_NAME"] = "Выводить название элемента";
$MESS["T_IBLOCK_DESC_NEWS_PICTURE"] = "Выводить изображение для анонса";
$MESS["T_IBLOCK_DESC_NEWS_TEXT"] = "Выводить текст анонса";
$MESS["T_IBLOCK_DESC_IMG_WIDTH"] = "Ширина картинки для анонса";
$MESS["T_IBLOCK_DESC_IMG_HEIGHT"] = "Высота картинки для анонса";
$MESS["T_IBLOCK_DESC_LINE_NEWS_COUNT"] = "Количество элементов выводимых в одной строке таблицы";
$MESS["T_IBLOCK_NAME_BLOCK"] = "Заголовок для блока с новостями от пользователей";
$MESS["T_IBLOCK_DESC_NAME_BLOCK"] = "Народные новости";
?>